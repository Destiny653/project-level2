    ...
    return (
        <>
            ...
            <div className='flex gap-10  w-full bg-slate-100 box-border p-5'>
                ...
                <section className='w-10/12 flex flex-col gap-7 '>
                    <div>
                    ...
                    <div>
                        {
                            display == false ?
                                data.map((item, index) => (
                                    <div key={item._id} style={mode == 'black' ? { color: 'black' } : { color: 'black' }} className='card-hover  flex  items-center border-2  box-border w-full gap-2 p-3 rounded-xl mb-7 bg-white relative overflow-hidden '>
                                        <div className=' w-2/6'>
                                            <Link href={`/${item._id}`}>
                                                <Image className=' w-full' src={item.img} alt='content image' width={200} height={200} />
                                            </Link>
                                        </div>
                                        <div className='roboto flex flex-col gap-4 pr-11 w-2/3 '>
                                            <h1 className=' text-xs nunito font-normal ' style={{ color: 'gray' }}>{item.title.toUpperCase()}</h1>
                                            <p className=' text-base'>{item.description} Lorem ipsum dolor sit amet consectetur, adipisicing elit. Dolore libero harum dolorem sed et adipisci
                                                dolores mollitia quod placeat! Consequatur voluptas error quibusdam voluptatibus. Illo quaerat non nobis ea asperiores.</p>
                                            <h1>
                                            ...
                                            <button onClick={() => {handleAddToCart(item); setIndexval(index); setSelected(true)}} className=' btn-bg text-base roboto font-semibold bg-neutral-100 px-9 py-3 rounded-lg roboto'>ADD TO CART</button>
                                ...
                                <div className='grid grid-cols-3 gap-7 '>
                                    ...
                                    {data.map((item, index) => {
                                        ...
                                        return (
                                            <div key={item._id} style={mode == 'black' ? { color: 'black' } : { color: 'black' }} className='item card-hover  flex flex-col justify-center items-center gap-3 box-border p-4 rounded-2xl relative overflow-hidden bg-white'>
